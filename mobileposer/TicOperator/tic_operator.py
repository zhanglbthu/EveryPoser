...
class TicOperator():
    def __init__(self, TIC_network, imu_num=6, ego_imu_idx=-1, data_frame_rate=30, device=None):
        self.buffer_size = config.tic_ws
        self.TR_drift = torch.Tensor([10, 10, 0]) * 1
        self.TR_offset = torch.Tensor([30, 30, 15]) * 1
    ...
    def dynamic_calibration(self):
        if len(self.data_buffer) < config.tic_ws:
            return
        frame_nums = config.tic_ws
